...
Partial Class FormMonitoringDekubitus
    Inherits System.Web.UI.Page
    Private Sub getDataHdr()
        Dim strsql As String = ""
        'Monitoring_hdr_id()
        strsql = "SELECT * from Inos_Monitoring_DEKUBITUS " & _
        ...
        While dr.Read
            ...
            If IsDBNull(dr("bt_infeksi_DEKUBITUS")) Then
                ChkBoxInfeksiDEKUBITUSTidak.Checked = True
            Else
                If (dr("bt_infeksi_DEKUBITUS") = True) Then
                    ChkBoxInfeksiDEKUBITUSYa.Checked = True
                Else
                    ChkBoxInfeksiDEKUBITUSTidak.Checked = True
                End If
            End If
            If IsDBNull(dr("vc_area")) Then
                txtArea.Text = ""
            Else
                txtArea.Text = dr("vc_area")
            End If
        End While
    ...
    Private Sub getDataPencegahanDEKUBITUS()
        Dim connectionString As String = WebConfigurationManager.ConnectionStrings("koneksi").ConnectionString
        Dim connection As SqlConnection = New SqlConnection(connectionString)
        ...
        If (getMonitoringHdrId() <> 0) Then
            strsql = "SELECT  DEKUBITUS.*, item.vc_nama,in_monitoring_hdr_id,item.vc_kode, vc_tgl_1,vc_tgl_2,vc_tgl_3, " & _
                             ...
                             "vc_tgl_21,vc_tgl_22,vc_tgl_23,vc_tgl_24,vc_tgl_25,vc_tgl_26,vc_tgl_27,vc_tgl_28,vc_tgl_29,vc_tgl_30,vc_tgl_31 FROM Inos_Monitoring_DEKUBITUS DEKUBITUS LEFT JOIN  Inos_Monitoring_Trans trans " & _
                             "on trans.in_monitoring_hdr_id = DEKUBITUS.in_monitoring_id " & _
                             "RIGHT JOIN Inos_monitoring_Item item " & _
                             "ON item.vc_kode = trans.vc_kode " & _
                              "and item.vc_source = trans.vc_source " & _
                             "and DEKUBITUS.in_monitoring_id= '" & getMonitoringHdrId() & "' " & _
                             "where item.vc_source = 'DEKUBITUS' " & _
                             "and vc_type = 'Pencegahan' " & _
                               "and item.bt_active = 1 " & _
        ...
        Else
            strsql = "SELECT DEKUBITUS.*, item.vc_nama,in_monitoring_hdr_id,item.vc_kode, vc_tgl_1,vc_tgl_2,vc_tgl_3, " & _
                     ...
                     "ON item.vc_kode = trans.vc_kode " & _
                      "and item.vc_source = trans.vc_source " & _
                     "and item.vc_source = 'DEKUBITUS' " & _
                     "and trans.in_monitoring_hdr_id = 0 " & _
                     "LEFT JOIN Inos_Monitoring_DEKUBITUS DEKUBITUS " & _
                     "on DEKUBITUS.in_monitoring_id = trans.in_monitoring_hdr_id " & _
    ...
    Private Sub getDataDiagnosaDEKUBITUS()
        Dim connectionString As String = WebConfigurationManager.ConnectionStrings("koneksi").ConnectionString
        Dim connection As SqlConnection = New SqlConnection(connectionString)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If (Not Page.IsPostBack) Then
            getDataHdr()
            getDataPencegahanDEKUBITUS()
            getDataDiagnosaDEKUBITUS()
        End If
    ...
    Protected Sub BtnSaveMonitoringDEKUBITUS_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnSaveMonitoringDEKUBITUS.Click
        SaveHdr()
        DeleteOldData()
        saveDataPencegahanDEKUBITUS()
        saveDataDiagnosaDEKUBITUS()
    ...
    Private Sub DeleteOldData()
        ...
        myCommand.Parameters("@source_id").Value = "DEKUBITUS"
    ...
    Private Sub SaveHdr()
        ...
        If (Me.ChkBoxInfeksiDEKUBITUSYa.Checked) Then
            infeksi = True
        End If

        Try
            If (getMonitoringHdrId() <> 0) Then
                strsql = "update Inos_Monitoring_DEKUBITUS set " & _
                         " bt_infeksi_DEKUBITUS =  '" & infeksi & "' " & _
                         " ,vc_area =  '" & txtArea.Text & "' " & _
                         " where in_transaction_id =  '" & Request.QueryString("transactionId") & "' "
            Else
                strsql = "insert into Inos_Monitoring_DEKUBITUS (in_transaction_id,  bt_infeksi_DEKUBITUS,vc_area  )  " & _
                                                                          " values " & _
                                                     "('" & Request.QueryString("transactionId") & "', '" & infeksi & "', '" & txtArea.Text & "')"
    ...
    End Sub
    Private Sub saveDataPencegahanDEKUBITUS()
        Dim strsql As String = ""
        Dim connectionString As String = WebConfigurationManager.ConnectionStrings("koneksi").ConnectionString
        Dim connection As SqlConnection = New SqlConnection(connectionString)
        Dim command As SqlCommand = New SqlCommand()
                ...
                Try
                    strsql = "insert into Inos_Monitoring_Trans (in_monitoring_hdr_id ,vc_kode,vc_source,vc_tgl_1,vc_tgl_2,vc_tgl_3,vc_tgl_4,vc_tgl_5,vc_tgl_6, " & _
                                                       ...
                                                       "vc_tgl_26,vc_tgl_27,vc_tgl_28,vc_tgl_29,vc_tgl_30,vc_tgl_31) " & _
                                                                           " values " & _
                                                      "('" & getMonitoringHdrId() & "','" & vc_kode & "','DEKUBITUS','" & tgl_1 & "','" & tgl_2 & "','" & tgl_3 & "','" & tgl_4 & "','" & tgl_5 & "','" & tgl_6 & "' " & _
    ...
    Private Sub saveDataDiagnosaDEKUBITUS()
        Dim strsql As String = ""
        Dim connectionString As String = WebConfigurationManager.ConnectionStrings("koneksi").ConnectionString
        Dim connection As SqlConnection = New SqlConnection(connectionString)
        Dim command As SqlCommand = New SqlCommand()
