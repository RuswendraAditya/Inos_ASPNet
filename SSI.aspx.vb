...
Partial Class SSI
    ...
    Private Sub LapHais()
        ...
        dthasil.Columns.Add("SSI")                         'Ambil NAMA
        ...
        While objdatareader.Read
            For j = 0 To dthasil.Rows.Count - 1
                If dthasil.Rows(j).Item("kode") = "0006" Then    'Kotor
                    If objdatareader("in_lama_kotor") > 0 Then
                        dthasil.Rows(j).Item("lama") = objdatareader("in_lama_kotor")
                    End If
                    dthasil.Rows(j).Item("JUMLAH") = Val(dthasil.Rows(j).Item("JUMLAH")) + objdatareader("dc_operasi_kotor_hasil")
                End If
                If dthasil.Rows(j).Item("kode") = "0005" Then    'BT
                    If objdatareader("in_lama_bt") > 0 Then
                        dthasil.Rows(j).Item("lama") = objdatareader("in_lama_bt")
                    End If
                    dthasil.Rows(j).Item("JUMLAH") = Val(dthasil.Rows(j).Item("JUMLAH")) + objdatareader("dc_operasi_bt_hasil")
                End If

                If dthasil.Rows(j).Item("kode") = "0004" Then    'Bersih
                    If objdatareader("in_lama_bersih") > 0 Then
                        dthasil.Rows(j).Item("lama") = objdatareader("in_lama_bersih")
                    End If
                    dthasil.Rows(j).Item("JUMLAH") = Val(dthasil.Rows(j).Item("JUMLAH")) + objdatareader("dc_operasi_bersih_hasil")
                End If
        ...
        For j = 0 To dthasil.Rows.Count - 1
            If dthasil.Rows(j).Item("JUMLAH") <> 0 Then
                Dim njumlah As Single = 0
                Dim nLama As Integer = 0
                njumlah = dthasil.Rows(j).Item("JUMLAH")
                nLama = dthasil.Rows(j).Item("LAMA")
                dthasil.Rows(j).Item("HASIL") = Format(njumlah / nLama * 100, "standard")
                'dthasil.Rows(j).Item("HASIL") = (Val(dthasil.Rows(j).Item("JUMLAH")) / Val(dthasil.Rows(j).Item("LAMA"))) * 100
            End If
        Next
