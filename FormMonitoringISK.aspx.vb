...
Partial Class FormMonitoringISK
    Inherits System.Web.UI.Page
    Private Sub getDataHdr()
        Dim strsql As String = ""
        'Monitoring_hdr_id()
        strsql = "SELECT * from Inos_Monitoring_ISK " & _
        ...
        While dr.Read
            ...
            If IsDBNull(dr("bt_silikon")) Then
                chkBoxSilikon.Checked = False
            Else
                If (dr("bt_silikon") = True) Then
                    chkBoxSilikon.Checked = True
                Else
                    chkBoxSilikon.Checked = False
                End If
            End If
            If IsDBNull(dr("bt_folley")) Then
                chkBoxFolley.Checked = False
            Else
                If (dr("bt_folley") = True) Then
                    chkBoxFolley.Checked = True
                Else
                    chkBoxFolley.Checked = False
                End If
            End If
            If IsDBNull(dr("bt_cateter_10")) Then
                chkBoxCateter10.Checked = False
            Else
                If (dr("bt_cateter_10") = True) Then
                    chkBoxCateter10.Checked = True
                Else
                    chkBoxCateter10.Checked = False
                End If
            End If
            If IsDBNull(dr("bt_cateter_12")) Then
                chkBoxCateter12.Checked = False
            Else
                If (dr("bt_cateter_12") = True) Then
                    chkBoxCateter12.Checked = True
                Else
                    chkBoxCateter12.Checked = False
                End If
            End If
            If IsDBNull(dr("bt_cateter_14")) Then
                chkBoxCateter14.Checked = False
            Else
                If (dr("bt_cateter_14") = True) Then
                    chkBoxCateter14.Checked = True
                Else
                    chkBoxCateter14.Checked = False
                End If
            End If
            ...
            Else
                ...
                End If
            End If
            If IsDBNull(dr("bt_infeksi_isk")) Then
                ChkBoxInfeksiISKTidak.Checked = True
            Else
                If (dr("bt_infeksi_isk") = True) Then
                    ChkBoxInfeksiISKYa.Checked = True
                Else
                    ChkBoxInfeksiISKTidak.Checked = True
                End If
            End If
    ...
    Private Sub getDataPencegahanISK()
        Dim connectionString As String = WebConfigurationManager.ConnectionStrings("koneksi").ConnectionString
        Dim connection As SqlConnection = New SqlConnection(connectionString)
        ...
        If (getMonitoringHdrId() <> 0) Then
            strsql = "SELECT  ISK.*, item.vc_nama,in_monitoring_hdr_id,item.vc_kode, vc_tgl_1,vc_tgl_2,vc_tgl_3, " & _
                             ...
                             "vc_tgl_21,vc_tgl_22,vc_tgl_23,vc_tgl_24,vc_tgl_25,vc_tgl_26,vc_tgl_27,vc_tgl_28,vc_tgl_29,vc_tgl_30,vc_tgl_31 FROM Inos_Monitoring_ISK ISK LEFT JOIN  Inos_Monitoring_Trans trans " & _
                             "on trans.in_monitoring_hdr_id = ISK.in_monitoring_id " & _
                             "RIGHT JOIN Inos_monitoring_Item item " & _
                             "ON item.vc_kode = trans.vc_kode " & _
                              "and item.vc_source = trans.vc_source " & _
                             "and ISK.in_monitoring_id= '" & getMonitoringHdrId() & "' " & _
                             "where item.vc_source = 'ISK' " & _
        ...
        Else
            strsql = "SELECT ISK.*, item.vc_nama,in_monitoring_hdr_id,item.vc_kode, vc_tgl_1,vc_tgl_2,vc_tgl_3, " & _
                     ...
                     "ON item.vc_kode = trans.vc_kode " & _
                      "and item.vc_source = trans.vc_source " & _
                     "and item.vc_source = 'ISK' " & _
                     "and trans.in_monitoring_hdr_id = 0 " & _
                     "LEFT JOIN Inos_Monitoring_ISK ISK " & _
                     "on ISK.in_monitoring_id = trans.in_monitoring_hdr_id " & _
    ...
    Private Sub getDataDiagnosaISK()
        Dim connectionString As String = WebConfigurationManager.ConnectionStrings("koneksi").ConnectionString
        Dim connection As SqlConnection = New SqlConnection(connectionString)
    ...
    Protected Sub BtnSaveMonitoringISK_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnSaveMonitoringISK.Click
        SaveHdr()
        DeleteOldData()
        saveDataPencegahanISK()
        saveDataDiagnosaISK()
    ...
    Private Sub DeleteOldData()
        ...
        myCommand.Parameters("@source_id").Value = "ISK"
    ...
    Private Sub SaveHdr()
        ...
        If (Me.ChkBoxInfeksiISKYa.Checked) Then
            infeksi = True
        End If

        Try
            If (getMonitoringHdrId() <> 0) Then
                strsql = "update Inos_Monitoring_ISK set " & _
                         "bt_silikon = '" & Me.chkBoxSilikon.Checked & "' " & _
                         ",bt_folley = '" & Me.chkBoxFolley.Checked & "' " & _
                         ",bt_cateter_10 = '" & Me.chkBoxCateter10.Checked & "' " & _
                         ",bt_cateter_12 =    '" & Me.chkBoxCateter12.Checked & "' " & _
                         ",bt_cateter_14 =    '" & Me.chkBoxCateter14.Checked & "' " & _
                         ...
                         ",  bt_infeksi_isk =  '" & infeksi & "' " & _
                         " where in_transaction_id =  '" & Request.QueryString("transactionId") & "' "
            Else
                strsql = "insert into Inos_Monitoring_ISK (in_transaction_id, bt_silikon,bt_folley,bt_cateter_10, " & _
                                                      "bt_cateter_12, bt_cateter_14, bt_cateter_16, " & _
                                                      "bt_cateter_18,bt_cateter_20, bt_cateter_22, bt_infeksi_isk  )  " & _
                                                                          " values " & _
                                                     "('" & Request.QueryString("transactionId") & "','" & Me.chkBoxSilikon.Checked & "','" & Me.chkBoxFolley.Checked & "','" & Me.chkBoxCateter10.Checked & "' " & _
                                                     ",'" & Me.chkBoxCateter12.Checked & "','" & Me.chkBoxCateter14.Checked & "','" & Me.chkBoxCateter16.Checked & "'" & _
                                                     ",'" & Me.chkBoxCateter18.Checked & "','" & Me.chkBoxCateter20.Checked & "','" & Me.chkBoxCateter22.Checked & "','" & infeksi & "')"
    ...
    End Sub
    Private Sub saveDataPencegahanISK()
        Dim strsql As String = ""
        Dim connectionString As String = WebConfigurationManager.ConnectionStrings("koneksi").ConnectionString
        Dim connection As SqlConnection = New SqlConnection(connectionString)
        Dim command As SqlCommand = New SqlCommand()
                ...
                Try
                    strsql = "insert into Inos_Monitoring_Trans (in_monitoring_hdr_id ,vc_kode,vc_source,vc_tgl_1,vc_tgl_2,vc_tgl_3,vc_tgl_4,vc_tgl_5,vc_tgl_6, " & _
                                                       ...
                                                       "vc_tgl_26,vc_tgl_27,vc_tgl_28,vc_tgl_29,vc_tgl_30,vc_tgl_31) " & _
                                                                           " values " & _
                                                      "('" & getMonitoringHdrId() & "','" & vc_kode & "','ISK','" & tgl_1 & "','" & tgl_2 & "','" & tgl_3 & "','" & tgl_4 & "','" & tgl_5 & "','" & tgl_6 & "' " & _
    ...
    Private Sub saveDataDiagnosaISK()
        Dim strsql As String = ""
        Dim connectionString As String = WebConfigurationManager.ConnectionStrings("koneksi").ConnectionString
        Dim connection As SqlConnection = New SqlConnection(connectionString)
        Dim command As SqlCommand = New SqlCommand()
